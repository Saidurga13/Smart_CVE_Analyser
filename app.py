    ...
    if not cve_list:
        ...
        for (platform, cve), pkgs in cve_data.items():
            with st.expander(f"{platform} → {cve}"):
                ...
                st.markdown("```yaml\n" + "\n".join(grouped_output) + "\n```")
        # 🔁 Generate and display updated packages.json
        ...
        st.success(f"📦 The fixed version package list for product **{product_name}** is ready!")
        st.markdown(f"✅ `update_packages.json` contains all applicable CVE fixes from Agent2 for **{product_name}**.")

        st.subheader("🆕 Updated Packages JSON")
        st.json(updated_json)# 🔁 Generate and display updated packages.json